...
interface SummariesContextType {
  ...
  loading: boolean;
  fetchSummaries: (page: number, limit: number, title: string, setTotal: (total: number) => void) => Promise<void>;
  fetchMySummaries: (page: number, limit: number, setTotal: (total: number) => void) => Promise<void>;
...
export const SummariesProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    const fetchSummaries = async (page: number, limit: number, title = '', setTotal: (total: number) => void) => {
        try {
            setLoading(true);
    ...
    const fetchMySummaries = async (page: number, limit: number, setTotal: (total: number) => void) => {
        try {
            setLoading(true);
    ...
    return (
        <SummariesContext.Provider value={{ summaries, mySummaries, summary, loading, fetchSummaries, fetchMySummaries, fetchSummaryById, addSummary, updateSummary, deleteSummary }}>
        {children}
